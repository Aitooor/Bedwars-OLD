...
public class MySQL implements Database {
    ...
    public PlayerStats fetchStats(final UUID uuid) {
        ...
        final String s = "SELECT first_play, last_play, wins, kills, final_kills, looses, deaths, final_deaths, beds_destroyed, games_played, deathCryName, killMessageName, sprayName, woodSkin, finalKillName, topperName, shopkeeperName, bedDestroyName FROM global_stats WHERE uuid = ?;";
        try (final Connection connection = this.dataSource.getConnection();
             final PreparedStatement prepareStatement = connection.prepareStatement(s)) {
            prepareStatement.setString(1, uuid.toString());
            try (final ResultSet executeQuery = prepareStatement.executeQuery()) {
                if (executeQuery.next()) {
                    ...
                    playerStats.setDeathCry(BedWars.getInstance().getDeathCriesManager().getDeathCry(executeQuery.getString(11)));
                    playerStats.setKillMessage(BedWars.getInstance().getKillMessagesManager().getKillMessage(executeQuery.getString(12)));
                    playerStats.setSpray(BedWars.getInstance().getSpraysManager().getSpray(executeQuery.getString(13)));
                    playerStats.setWoodSkin(BedWars.getInstance().getWoodSkinsManager().getWoodSkin(executeQuery.getString(14)));
                    playerStats.setFinalKill(BedWars.getInstance().getFinalKillsManager().getFinalKill(executeQuery.getString(15)));
                    playerStats.setTopper(BedWars.getInstance().getToppersManager().getTopper(executeQuery.getString(16)));
                    playerStats.setShopkeeper(BedWars.getInstance().getShopkeeperManager().getShopkeeper(executeQuery.getString(17)));
                    playerStats.setBedDestroy(BedWars.getInstance().getBedDestroysManager().getBedDestroy(executeQuery.getString(18)));
                }
            }
        } catch (SQLException sqlException) {
            sqlException.printStackTrace();
