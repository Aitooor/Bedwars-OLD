...
import dev.eugenio.nasgarbedwars.arena.compass.util.VersionUtil;
import dev.eugenio.nasgarbedwars.api.configuration.ConfigManager;
import dev.eugenio.nasgarbedwars.api.menu.MenuType;
...
public class ConfigData extends ConfigManager {
    public ConfigData(Plugin plugin, String name, String dir) {
        ...
        config.options().header("Configuración de brújula.");

        config.addDefault(USE_COMMUNICATIONS, true);
        config.addDefault(MESSAGE_SEND_SOUND, BedWars.getInstance().getApi().getForCurrentVersion("ARROW_HIT", "ENTITY_ARROW_HIT_PLAYER", "ENTITY_ARROW_HIT_PLAYER") + ",1,1");
        ...
        saveItem(TRACKER_MENU_TEAM_ITEM, BedWars.getInstance().getApi().getForCurrentVersion("WOOL", "WOOL", "WHITE_WOOL"), false);
        ...
        if (isFirstTime()) {
            ...
            saveCommunicationItem("3", BedWars.getInstance().getApi().getForCurrentVersion("IRON_FENCE", "IRON_FENCE", "IRON_BARS"), false, 12, MenuType.NONE);
            ...
            saveCommunicationItem("9", BedWars.getInstance().getApi().getForCurrentVersion("IRON_FENCE", "IRON_FENCE", "IRON_BARS"), false, 22, MenuType.NONE);
